//Challenge 1
//Write a function that accepts an array of student objects, as shown below. Print all of the students' names and their cohorts.

function printCohort(array){
    for(var i = 0; i < array.length; i++){
        console.log("Name: " + array[i].name + ", Cohort: " + array[i].cohort);
    }
}

let students = [
    {name: 'Remy', cohort: 'Jan'},
    {name: 'Genevieve', cohort: 'March'},
    {name: 'Chuck', cohort: 'Jan'},
    {name: 'Osmund', cohort: 'June'},
    {name: 'Nikki', cohort: 'June'},
    {name: 'Boris', cohort: 'June'}
];

printCohort(students);

// Challenge 2
// Write a function that accepts an object of users divided into employees and managers, as shown below, and logs the information to the console.
function printUsers(object){
    for(let employee in users){
        console.log(employee.toUpperCase());
        for(let i = 0; i < users[employee].length; i++){
            let charactersCounter = users[employee][i].last_name.length + users[employee][i].first_name.length;
            console.log(i+1 + " - " + users[employee][i].last_name.toUpperCase() + ", " + users[employee][i].first_name.toUpperCase() + " - " + charactersCounter)
        }
    }
}

let users = {
    employees: [
        {'first_name':  'Miguel', 'last_name' : 'Jones'},
        {'first_name' : 'Ernie', 'last_name' : 'Bertson'},
        {'first_name' : 'Nora', 'last_name' : 'Lu'},
        {'first_name' : 'Sally', 'last_name' : 'Barkyoumb'}
    ],
    managers: [
        {'first_name' : 'Lillian', 'last_name' : 'Chambers'},
        {'first_name' : 'Gordon', 'last_name' : 'Poe'}
    ]
};

printUsers(users);